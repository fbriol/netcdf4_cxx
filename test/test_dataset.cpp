...
BOOST_AUTO_TEST_CASE(test_container) {
  ...
  for (short i = 0; i < static_cast<short>(strings.size()); ++i) {
    netcdf::Attribute(object, strings[i])
        .Write(netcdf::type::Short(object), std::vector<short>({i}));
  }
...
BOOST_AUTO_TEST_CASE(test_case) {
  Object object;
  netcdf::Attribute(object, "Attribute")
      .Write(netcdf::type::Short(object), std::vector<short>({1}));
  netcdf::DataSet ds(object);
